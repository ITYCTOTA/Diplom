...
import type { Friend, FriendRequest } from '@/entities/friendship'
...
export function useGameHubState() {
  ...
  const updateGameRating = useCallback((gameId: string, rating: number | null) => {
    setGames((current) =>
      current.map((game) => (game.id === gameId ? { ...game, rating } : game)),
    )
    setRecommendationGames((current) =>
      current.map((game) => (game.id === gameId ? { ...game, rating } : game)),
    )
  }, [])
